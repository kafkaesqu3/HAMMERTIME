...
class EncryptAES(Stage): 
    def run_command(self):
        print(self.previous_stage)
        bytes = open(self.previous_stage, 'rb').read()
        c = AesEncryption_Internal()
        c.base64 = False
        encrypted = c.encrypt(bytes, self.params["password"])
        return encrypted

    def build(self, build_directory):
        encrypted = self.run_command()
        out_file = "{}/shellcode.bin.encrypted".format(build_directory)
        output = open(out_file, "wb")
        output.write(encrypted)
        return out_file

class AesEncryption_Internal(object):
    '''
